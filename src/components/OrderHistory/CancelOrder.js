...
const CancelOrder = () => {
  ...
  const order = async () => {
    setIsLoading(true);
    try {
      const data = await axios.get("/api/my-orders/CANCELLED?page=0&limit=10");
  ...
  const filterdata = async (selectedValue) => {
    ...
    try {
      const data = await axios.get(
        `api/user-order-filter?filterType=${filterType}&orderType=CANCELLED`
      );
      console.log(data);
      setOrderList(data?.data?.data);
      setIsLoading(false);
                    ...
                    <>
                      <div className="order">
                        <div className="orderhead">
                          ...
                          <div>
                            <h5 className="dilivery-status-cancel">
                              {item?.status}
                            </h5>
                          </div>
                        </div>

...
export default function Input(props) {
  const { error, type, phoneInput ,setFieldValue,setIsLoading} = props;
 ...
 const handleChange = async(event) => {
   ...
   if (fileSize > 2) {
         alert("ex_2mb");
           ...
           setIsLoading(true);
var video = await uploadImage(event.target.files[0]);
                                                                
                                      
setFieldValue("video", video.path);
  ...
  return (
    <>
        {!fileObject ? (
            <>
               <Newdiv >
        <Videobutton type="button" onClick={handleClick}>Click or drag a file to this area to upload .</Videobutton>
      </Newdiv>
      <input
      {...props}
      name='video'
      ...
      />
            ...
            <>
               <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"0.5rem"}}>
                <div style={{padding:"1rem",backgroundColor:"#F9F9F9",borderRadius:"1rem",color:"#787878",border:"1px solid #CECECE"}}>
                  {fileObject.name}
                  </div>
                  <CrossButton type='button' onClick={handleBlank}>x</CrossButton>
               </div>
            </>
        )}
        {error ? (
